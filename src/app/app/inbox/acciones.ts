...
export async function enviarComoHumano(conversacionId: string, texto: string) {
  ...
  const { data: canal } = await db
    ...
    .maybeSingle()
    .overrideTypes<{ phone_number: string; ycloud_credential_ref: string | null }, { merge: false }>();
  ...
  if (!canal || !contacto) return { ok: false, error: "Canal o contacto no encontrado" };

  const apiKey = await leerSecreto(canal.ycloud_credential_ref);
  if (!apiKey) return { ok: false, error: "El canal no tiene credenciales de YCloud" };
